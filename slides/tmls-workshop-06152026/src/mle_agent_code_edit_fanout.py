"""Autoresearch with a fan-out code-editing MLE agent (CODE MODE).

Like :mod:`mle_agent_code_edit`, but the agent orchestrates **batches** of
hypotheses in parallel:

1. Plan a batch of code edits (``record_batch_plan``)
2. Save each ``train.py`` edit (``edit_train_code_batch`` or ``edit_train_code``)
3. Run experiments concurrently (``run_experiment_batch`` or ``flyte_map``)
4. Rank outcomes (``evaluate_batch_results``) and iterate

``code_mode=True`` lets the agent write Python plans with loops and
``flyte_map("run_experiment", ...)`` fan-out while tools stay durable Flyte tasks.

Run::

    uv run python mle_agent_code_edit_fanout.py --n-experiments 6 --batch-size 3
"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fan-out code-edit autoresearch agent (CODE MODE)")
